...
turtles-own [
  ...
  vision          ;; the distance that this turtle can see in the horizontal and vertical directions (transfer?)
  vision-points   ;; the points that this turtle can see in relative to it's current position (based on vision)
  age
]

patches-own [       ;; the amount of sugar on this patch
     psugar  ;; the maximum amount of sugar that can be on this patch
]
...
to patch-setup
  ask patches
  [ ifelse pcolor = green
    [set psugar random-in-range 500 max-psugar ]
    [set psugar 0]]
end
...
to turtle-setup ;; turtle procedure
  ...
  set shape "circle"
  set size 0.30
  set sugar random-in-range min-ind-sugar max-ind-sugar
  set metabolism random-in-range 1 2
...
to turtle-eat ;; turtle procedure
  ...
  ifelse pcolor = green
  [set sugar (sugar - metabolism + (psugar / count turtles))
    set psugar (psugar - (psugar / count turtles))]
  [set sugar (sugar - metabolism)
    set psugar 0]

 if ticks mod 25 = 0 [
   ask n-of (count turtles * percentage-of-transfer / 100) turtles [set sugar (sugar + 20)]
  ]
end
